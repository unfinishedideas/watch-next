...
app.MapPost("watch-lists/{id}/medias/{media_id}", (Guid id, Guid media_id) => wAPI.AddMediaToWatchList(id, media_id));
app.MapDelete("watch-lists/{id}/medias/{media_id}", (Guid id, Guid media_id) => wAPI.RemoveMediaFromWatchList(id, media_id));
app.MapPut("watch-lists/{id}/medias/{mov_id}", (Guid id, Guid mov_id, int new_spot) => wAPI.ReorderWatchListMedia(id, mov_id, new_spot));
app.MapPut("watch-lists/medias/update", (WatchListMediaUpdate req) => wAPI.UpdateWatchListMedia(req));
app.MapDelete("watch-lists/{id}", (Guid id) => wAPI.DeleteWatchList(id));

// Medias
app.MapPost("medias/", (MediaRegister req) => mAPI.CreateMedia(req));
app.MapGet("medias/", () => mAPI.GetAllMedias());
app.MapGet("medias/{id}/", (Guid id) => mAPI.GetMediaById(id));
app.MapGet("medias/title/{title}/", (string title) => mAPI.GetMediasByTitle(title));
app.MapGet("medias/director/{director}/", (string director) => mAPI.GetMediasByDirector(director));
app.MapGet("medias/genre/{genre}/", (string genre) => mAPI.GetMediasByGenre(genre));
app.MapGet("medias/year/{year}/", (int year) => mAPI.GetMediasByYear(year));
